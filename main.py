...
def audio_to_wav(file, rate: 16000):
    ...
    with av.open(file) as in_container:
        ...
        with av.open(wav, "w", "wav") as out_container:
            out_stream = out_container.add_stream("pcm_s16le", rate=rate, layout="mono")
...
if device == "cuda":
    ...
    for cuda_dev_num in range(0, cuda_num_devices, 1):
        ...
        cuda_device_capability = functools.reduce(
            lambda sub, ele: sub * 10 + ele, cuda_device_capability
        )
        ...
        logger.info(
            f"CUDA: Device {cuda_dev_num} total memory: {cuda_total_memory} bytes"
        )
        logger.info(
            f"CUDA: Device {cuda_dev_num} free memory: {cuda_free_memory} bytes"
        )
        ...
        if cuda_free_memory <= settings.chunking_memory_threshold:
            logger.warning(
                f"CUDA: Device {cuda_dev_num} has low memory, disabling chunking support"
            )
            support_chunking = False

        if cuda_free_memory <= settings.sv_memory_threshold:
            logger.warning(
                f"CUDA: Device {cuda_dev_num} has low memory, disabling SV support"
            )
            support_sv = False
        ...
        if cuda_device_capability in range(1, 59):
            logger.warning(
                f"CUDA: Device {cuda_dev_num} is pre-Pascal, forcing float32"
            )
            logger.warning(
                "CUDA: SUPPORT FOR PRE-PASCAL DEVICES IS UNSUPPORTED AND WILL BE REMOVED IN THE FUTURE"
            )
...
if support_sv:
    ...
    sv_model = transformers.WavLMForXVector.from_pretrained(
        "./models/microsoft-wavlm-base-plus-sv"
    ).to(device=device)
else:
...
class LazyModels:
    ...
    def whisper_processor(self):
        if self._whisper_processor is None:
            self._whisper_processor = transformers.WhisperProcessor.from_pretrained(
...
def load_models() -> LazyModels:
    ...
    logger.info(
        f"CTRANSLATE: Supported compute types for device {device} are {supported_compute_types}"
...
def do_translate(whisper_model, features, total_chunk_count, language, beam_size):
    ...
    task = "translate"
    logger.debug(
        f"WHISPER: Doing translation with {language}, beam size {beam_size}, chunk count {total_chunk_count}"
    )
    ...
    results = whisper_model.generate(
        features, [prompt] * total_chunk_count, beam_size=beam_size
    )
    time_end = datetime.datetime.now()
    infer_time = time_end - time_start
    infer_time_milliseconds = infer_time.total_seconds() * 1000
    logger.debug(
        "WHISPER: Translate inference took " + str(infer_time_milliseconds) + " ms"
    )
...
def do_whisper(
    ...
    if audio_duration >= long_beam_size_threshold:
        logger.debug(
            f"WHISPER: Audio duration is {audio_duration} ms - activating long mode"
        )
    ...
    if audio_duration > 30 * 1000:
        if support_chunking:
            ...
            logger.warning(
                "WHISPER: Audio duration is > 30s but chunking is not available. Will truncate!"
            )
    ...
    infer_time_milliseconds = infer_time.total_seconds() * 1000
    logger.debug(
        "WHISPER: Feature extraction took " + str(infer_time_milliseconds) + " ms"
    )
    ...
    if detect_language and not force_language:
        ...
        logger.debug(
            f"WHISPER: Detected language {language} with probability {probability}"
        )
    ...
    if detect_language:
        logger.debug(
            "WHISPER: Language detection took " + str(infer_time_milliseconds) + " ms"
        )
    ...
    for i, mel_features_batch in enumerate(
        ...
        logger.debug(
            "WHISPER: Processing batch %s of expected %s",
            i + 1,
            len(mel_features) // concurrent_gpu_chunks + 1,
        )
    ...
    assert (
        len(results) == total_chunk_count
    ), "Result length doesn't match expected total_chunk_count"
    ...
    if use_chunking:
        ...
        tokens = [
            (results[i].sequences_ids[0], strides[i]) for i in range(total_chunk_count)
        ]
        tokens = find_longest_common_sequence(
            tokens, models.whisper_processor.tokenizer
        )
    else:
    ...
    if translate and len(total_chunk_count) > concurrent_gpu_chunks:
        ...
        logger.debug(
            f"WHISPER: Detected non-preferred language {language}, translating"
        )
...
def do_sv(audio_file, threshold=sv_threshold):
    ...
    audio_input = sv_feature_extractor(
        audio_wav.squeeze(0), return_tensors="pt", sampling_rate=audio_sr
    ...
    def on_datachannel(channel):
        ...
        def on_message(message):
            ...
            if message.type == "ping":
                ...
                logger.debug(
                    f"RTC DC: Debug Stop Vars model {model} beam size {beam_size} detect language {detect_language}"
                )
                logger.debug("RTC DC: Recording stopped")
                ...
                recorder.stop()
                logger.debug(
                    "RTC DC: Recorder stop took " + str(infer_time_milliseconds) + " ms"
                )
                # Tell client what we are doing
                send_dc_response(
                    channel,
                    "log",
                    f"Doing ASR with model {model} beam size {beam_size} detect language {detect_language} - please wait",
                )
                ...
                (
                    ...
                    infer_speedup,
                    audio_duration,
                ) = do_whisper(recorder.file, model, beam_size, task, detect_language)
                ...
                if translation:
                    send_dc_response(
                        channel,
                        "log",
                        f"ASR Translation from {language}:  {translation}",
                    )
                ...
                send_dc_response(
                    channel, "log", f"ASR Audio Duration: {audio_duration} ms"
                )
                send_dc_response(
                    channel,
                    "log",
                    f"ASR Speedup: {infer_speedup}x faster than realtime",
                )
                recorder = None
            else:
                send_dc_response(
                    channel, "error", f'unknown message type "{message.type}"'
                )
...
basic_auth_401_response = PlainTextResponse(
    "Invalid credentials", status_code=401, headers={"WWW-Authenticate": "Basic"}
)
...
app.mount(
    "/rtc", StaticFiles(directory="nginx/static/rtc", html=True), name="rtc_files"
)

# Mount static dir to serve files for dictation client
app.mount(
    "/dict", StaticFiles(directory="nginx/static/dict", html=True), name="dict_files"
)

# Expose audio mount in the event willow is configured to save
app.mount(
    "/audio", StaticFiles(directory="nginx/static/audio", html=True), name="audio_files"
)
...
@app.post(
    "/api/rtc/asr",
    summary="Return SDP for WebRTC clients",
    response_description="SDP for WebRTC clients",
)
async def rtc_asr(
    request: Request,
    response: Response,
    ...
    detect_language: Optional[bool] = detect_language,
...
@app.post(
    "/api/asr",
    response_model=ASR,
    summary="Submit audio file for ASR",
...
async def asr(
    ...
    # prof.enable()
    logger.debug(
        f"FASTAPI: Got ASR request for model {model} beam size {beam_size} "
    ...
    if force_language:
        if not check_language(force_language):
            logger.debug(
                "FASTAPI: Invalid force_language in request - returning HTTP 400"
            )
    ...
    # Do Whisper
    language, results, infer_time, translation, infer_speedup, audio_duration = (
        do_whisper(
            audio_file,
            model,
            beam_size,
...
@app.post(
    "/api/willow",
    response_model=ASR,
    summary="Stream Willow audio for ASR",
    response_description="ASR engine output",
)
async def willow(
    request: Request,
    ...
    logger.debug(
        f"FASTAPI: Got WILLOW request for model {model} beam size {beam_size} "
        f"language detection {detect_language}"
    )
    task = "transcribe"
    ...
    if force_language:
        if not check_language(force_language):
            logger.debug(
                "WILLOW: Invalid force_language in request - returning HTTP 400"
            )
            response = {"error": "Invalid force_language"}
            return JSONResponse(
                content=response, status_code=status.HTTP_400_BAD_REQUEST
    ...
    logger.debug(
        f"WILLOW: Audio information: sample rate: {sample_rate}, bits: {bits}, channel(s): {channel}, "
        f"codec: {codec}"
    )
    ...
    try:
        if codec == "pcm":
            ...
            audio_file = write_stream_wav(
                body, int(sample_rate), int(bits), int(channel)
            )
        elif codec == "wav":
